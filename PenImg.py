...
for j in range(size_y):  # iterates through y-axis of the image
    for i in range(size_x):  # iterates through x-axis of the image
        ...
        tmp = int(255 - int((math.sqrt(math.pow(pixelRGB[0], 2) + math.pow(pixelRGB[1], 2) + math.pow(pixelRGB[2],
                                                                                                      2))) / precalc * 255))  # calculate the length of the vector, normalize it and map it between 0 and 255 (part of the HSV-transformation, to get the greyscale value)
...
for j in range(size_y):
    ...
    for i in range(notWhiteFrom, notWhiteTo + 1):
        ...
        if thisPX != nextPX or (
            ...
            if edge == 1:
                target.write('G01 X%s Y%s Z%s \n' % ((k * pixelScale) + offset_x, (j * pixelScale) + offset_y,
                                                     thisPX))  # writes a simple G1 path with Z as modulation value
...
print 'Total length of path: %.2fm. Estimated time: %.2fh' % (
lengthOfPath2 / 1000, lengthOfPath2 / (feedrate * math.pow(60, 2)))
print 'Total length of optimized path: %.2fm. Estimated time: %.2fh' % (
lengthOfPath / 1000, lengthOfPath / (feedrate * math.pow(60, 2)))
